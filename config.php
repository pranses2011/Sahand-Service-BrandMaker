 ...
 * 🌍 تنظیمات عمومی
 * -------------------------------------------------- */
define('SAHAND_VERSION', '2.21.0');             // نسخه سیستم (۲.۲۱.۰ — قالب‌سازی همه‌جانبه: ① دکمه «🎭 قالب‌ساز» + «✨ طراحی با UI/UX Pro» روی کارت «هر» صفحه در تب صفحه‌های ویرایش برند — بازکردن چیدمان همان صفحه در قالب‌ساز (?brand_page=) و ذخیره مستقیم روی brand_pages + طراحی حرفه‌ای یک‌کلیکی با زمینه واقعی برند (نام در هیرو، تعداد دستگاه/مقاله) ② کلاس TemplateLibrary: کتابخانه ۱۷۰ قالب آماده = ۱۰ سبک واقعاً متفاوت (مدرن/کلاسیک/مینیمال/لوکس/فنی-تیره/شرکتی/مجله‌ای/پرانرژی/تبدیل‌محور/صمیمی) × ۱۷ نوع صفحه — سید خودکار غیرمخبر (قالب‌های دستی و پیش‌فرض‌ها هرگز لمس نمی‌شوند؛ فقط کمبودها درج می‌شوند → مسیر ارتقا بدون تغییر دیتابیس) ③ قالب‌ساز: دراپ‌داون نوع صفحه از ۶ به ۱۷ گزینه (همه انواع سایت برند) ④ ویزارد ساخت برند ۸ مرحله‌ای: دکمه «⏭️ رد کردن مرحله و ادامه» هنگام خطای هر مرحله + حالت بصری skipped — خطای یک مرحله دیگر کل ساخت را متوقف نمی‌کند ⑤ رفع دو باگ کشف‌شده با تست: بلوک pagination ناموجود در بلوپرینت blog دانش UI/UX (→ related-links) + کد مرده شخصی‌سازی نام برند در هیرو (عنوان‌های از پیش پر بلوپرینت را تایید نمی‌کرد) ⑥ بازنویسی templates.php: حذف کد سید تکراری (دو بلاک یکسان) + دکمه «↻ بازسازی کتابخانه» + قفل‌گذاری قالب‌های کتابلایه در برابر حذف + رنگ‌بندی پیش‌نمایش بر اساس پس‌زمینه بلوک‌ها)
