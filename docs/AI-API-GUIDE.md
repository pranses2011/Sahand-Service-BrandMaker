# 🤖 راهنمای کامل و مفصل API هوش مصنوعی سهند — نسخه ۳.۲.۰ «بی‌رقیب + متصل به اینترنت + تصویر و اخبار»
...
### 🌐 موتور نسخه ۳.۱ — جستجوی آنلاین وب (۶ ارائه‌دهنده با جایگزینی خودکار + Mojeek در v3.2)

| # | متد | مسیر | کار | کلید API |
|:-:|:---:|------|-----|:--------:|
| ۸ | POST | `/api/ai/web-search` | 🔎 جستجوی آنلاین اینترنت (۷ ارائه‌دهنده با جایگزینی خودکار) | ✅ |
...
### 🌟 موتور نسخه ۳.۲ — اخبار زنده + تصاویر مقاله (جدید)
...
## ۱۰) 🤖 ربات تلگرام متصل به دستیار — راه‌اندازی و استفاده (v2)
...
همه ۱۲ فرمان دستیار (مقاله، کلیدواژه، امتیاز، بهبود، عنوان، نیت، عیب‌یابی، کد خطا، برندها، اخبار) + 🌐 جستجوی وب + 🧪 تحقیق آنلاین + فرمان‌های مدیریتی:
...
> 🎴 فرمان‌های فارسی هم کار می‌کنند: `/مقاله`، `/اخبار`، `/جستجو`، `/تحقیق`، `/کلمات`، `/عنوان`، `/بهبود`، `/امتیاز`
