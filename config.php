 ...
 * 🌍 تنظیمات عمومی
 * -------------------------------------------------- */
define('SAHAND_VERSION', '2.22.0');             // نسخه سیستم (۲.۲۲.۰ — ریشه‌یابی کامل دو خطای استقرار + خطایاب v3.14 + ۱۴ سرویس تصویر: ① 🚨 ریشه قطعی «تست نهایی ناموفق: HTTP 500 خط ۱۶ index.php» و «صفحات خالی سایت برند»: ConfigGenerator فایل config.php مستقرشده را فقط با ثابت‌ها بازنویسی می‌کرد و توابع حیاتی (fetchFromAPI/postToAPI/e/fa_num/cdn_asset) را پاک می‌کرد → هر صفحه با «Call to undefined function fetchFromAPI()» فاتال؛ اکنون config تولیدی = ثابت‌ها + مجموعه کامل توابع (هم‌ارز قالب) + پشتیبانی CACHE_ENABLED و کش کهنه در قطعی شبکه ② 🚨 ریشه «استخراج کامل نشد — index.php یافت نشد»: راستی‌آمایی حجم ZIP روی سرور قبل از استخراج (آپلود ناقص → بارگذاری مجدد خودکار) + 🚑 نجات FTP کامل: اگر fileop استخراج بی‌صدا هیچ فایلی تولید نکرد، بسته محلی باز و همه فایل‌ها تک‌به‌تک با FTP مستقر می‌شوند ③ خطایاب v3.14: بودجه ۴۴→۱۳۰ث + راند نجات همیشه با بودجه تازه + سرچر مشترک (cooldown جینا حفظ) + تفکیک «شکست زیرساخت» از «شاهد منفی» (هاست بلاک دیگر باعث رد همه کدها نمی‌شود) + واریانت کد E-01↔E01 + الگوهای دوهرفی کامل (SE/HE/TE/BE) و چشمک LED + دیکشنری تلویزیون/مایکروویو (بک‌لایت/T-Con/webOS/مگنترون/...) + پنجره کد-محور برای دلایل/راه‌حل (رفع آلودگی زمینه) + کدها دیگر مدل حساب نمی‌شوند + KB: ال‌جی مایکروویو (SE/FE/TE/EE/PF) و تلویزیون webOS (101/105/137/142/152/324/501) — مجموع ۱۶۸ کد ④ سرویس تصویر v3.0: رجیستری ۷→۱۴ سرویس (+AI Horde رایگان، fal، Imagen 3، gpt-image-1، Ideogram، GetImg، Replicate) + رفع تست سلامت (تصویر ۶۴۰×۳۶۰ + متد GET صحیح) + گزارش سرویس به‌ازای هر تصویر + اعلان «کلید ثبت نشده» + هم‌سازی همه خروجی‌ها به ۱۶:۹ + endpoint جدید HuggingFace + SmartPipeline از سرویس انتخابی استفاده می‌کند + سقف نرخ جستجو ۶۰→۲۴۰ (کلمپ + مهاجرت)
...
/* --------------------------------------------------
 * 🆕 مهاجرت v2.22 — سقف نرخ جستجوی وب نصب‌های قدیمی
 * seed اولیه rate_per_hour=60 بود؛ جستجوی عمیق خطایاب هر تولید ۳۰-۴۰
 * کوئری می‌سوزاند و سقف ۶۰ بعد از ۱-۲ دستگاه همه جستجوها را fail-fast
 * می‌کرد (ریشه‌یابی «هیچ کدی از جستجوی اینترنتی تأیید نشد»).
 * فقط یک بار (cache/.schema_v222).
 * -------------------------------------------------- */
if (!defined('SAHAND_NO_DB_MIGRATE')) {
    try {
