 ...
 * 🌍 تنظیمات عمومی
 * -------------------------------------------------- */
define('SAHAND_VERSION', '2.19.1');             // نسخه سیستم (۲.۱۹.۱ — ریشه‌یابی خطای «محتوای مسیر سایت: خالی»: زنجیره آپلود+استخراج موفق بود اما listFiles قالب پاسخ قدیمی/ناشناخته را بی‌صدا به «پوشه خالی» ترجمه می‌کرد و راستی‌آمایی index.php شکست کاذب می‌خورد → ① listFiles چندقالبی (files/dirs جدید 11.138 + آرایه تخت قدیمی + API2 listfiles fallback) با ردیابی وضعیت ok/ok-empty/unparsed/error + کش درون‌درخواستی با ابطال جهش‌محور ② entryExists دو-لایه: استعلام مستقیم get_file_information + fallback لیست ③ uploadFile با overwrite=1 (ZIP قدیمی مانده از تلاش ناموفق، آپلود جدید را بی‌صدا رد می‌کرد) + راستی‌آزمایی succeeded/failed ④ fileOp بدون موفقیت کاذب data[0] غایب ⑤ دیاگنوستیک کامل: وجود ZIP روی سرور + وضعیت لیست + نمونه پاسخ خام)
