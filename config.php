 ...
 * 🌍 تنظیمات عمومی
 * -------------------------------------------------- */
define('SAHAND_VERSION', '2.20.0');             // نسخه سیستم (۲.۲۰.۰ — ریشه‌یابی خطای «تست نهایی ناموفق: پاسخ HTTP: 500» در آخرین مرحله استقرار: ① باگ قطعی: کامنت‌های C- مانند در htaccess.template — سینتکس نامعتبر Apache → خطای 500 (برای نصب دستی ZIP دائمی!) → اصلاح به # ② «Require all denied» فقط Apache 2.4 است — روی Apache 2.2 (سرورهای cPanel قدیمی) «Invalid command» → 500 قطعی → بلوک‌های دوسینتکس با IfModule mod_authz_core.c / معکوس (Order+Deny) ③ HtaccessGenerator سه‌سطحی: full (کامل) / safe (بدون Options و بلوک‌های Files) / minimal (فقط DirectoryIndex + 404 + URLهای تمیز) ④ خوددرمانی: در صورت 500 در تست نهایی، .htaccess خودکار به سطح سازگارتر تنزل و دوباره تست می‌شود — دیگر .htaccess هرگز استقرار را متوقف نمی‌کند ⑤ تست نهایی دوسکیم: fallback HTTP روی «هر» پاسخ غیرقابل‌قبول HTTPS (زیردامنه بدون SSL → HTTPS به vhost پیش‌فرض می‌افتد و 500 کاذب می‌گیرد) + ۳ تلاش با فاصله ⑥ دیاگنوستیک: نمونه پاسخ سرور + انتهای error_log در پیام خطا — ریشه واقعی لو می‌رود)
