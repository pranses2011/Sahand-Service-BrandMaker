...
class PersianGrammar
{
    ...
    public static function fix(string $content, bool $fixAgreement = true): array
    {
        ...
        foreach ($protected as $i => $chunk) {
            if ($i % 2 === 1) { // تگ pre/code
                /* 🛡 v1.2: نشانگر بدون رقم — قبلاً «GPR0» بود و تبدیل ارقام به فارسی
                   نشانگر را خراب می‌کرد و بلوک کد برای همیشه گم می‌شد! */
                $ph = "\u{2062}GPR" . str_repeat('Q', count($placeholders) + 1) . "\u{2062}";
        ...
        foreach ($tokens as $i => $token) {
            ...
            $token = self::fixStyle($token, $stats);
            $token = self::fixRedundancy($token, $stats);
        ...
        $stats['total_fixes'] = $stats['zwnj'] + $stats['punctuation'] + $stats['spelling'] + $stats['style']
    ...
    /**
     * 🎭 سبک نگارشی — حذف حشو و عبارت‌های اداری/محاوره‌ای (v1.2)
     * قواعد دفتر فارسی و فرهنگستان: «می‌باشد» ممنوع، فعل‌های ساده ترجیح دارند.
     */
    private static function fixStyle(string $text, array &$stats): string
    {
        static $map = null;
        if ($map === null) {
            $z = self::ZWNJ;
            ...
            $e = '(?![\p{L}\p{N}])';
            $map = [
                // «می‌باشد» ممنوع (قاعده معروف نگارش رسمی)
                $b . 'می' . $z . 'باشد' . $e => 'است',
                $b . 'میباشد' . $e => 'است',
                $b . 'می باشد' . $e => 'است',
                $b . 'می' . $z . 'باشند' . $e => 'هستند',
                $b . 'میباشند' . $e => 'هستند',
                // فعل‌های سنگین → ساده
                $b . 'می' . $z . 'گردد' . $e => 'می' . $z . 'شود',
                $b . 'می گردد' . $e => 'می' . $z . 'شود',
                $b . 'گردیده' . $e => 'شده',
                $b . 'گردید' . $e => 'شد',
                $b . 'می' . $z . 'گردند' . $e => 'می' . $z . 'شوند',
                // اداری‌زدگی
                $b . 'جهت' . $e => 'برای',
                $b . 'مابین' . $e => 'میان',
                $b . 'فی الحال' . $e => 'فعلاً',
                ...
                $b . 'چجوری' . $e => 'چگونه',
                $b . 'چیکار' . $e => 'چه کاری',
                $b . 'می‌خوام' . $e => 'می‌خواهم',
                $b . 'میخوام' . $e => 'می‌خواهم',
                $b . 'می‌خواید' . $e => 'می‌خواهید',
                $b . 'بذارید' . $e => 'بگذارید',
                $b . 'بزنید' . $e => 'اجرا کنید',
                $b . 'درست میشه' . $e => 'درست می‌شود',
                $b . 'خب' . $e => 'خوب',
                // املاهای درست فرهنگستان
                $b . 'متاسفانه' . $e => 'متأسفانه',
                $b . 'مساله' . $e => 'مسئله',
                $b . 'خصوصیت' . $e => 'ویژگی',
                $b . 'تأسثیر' . $e => 'تأثیر',
                $b . 'متأسثر' . $e => 'متأثر',
    ...
    /**
     * 🔁 حشو و تکرار — واژه‌های پشت‌سرهم و فاصله‌های اضافی (v1.2)
     */
    private static function fixRedundancy(string $text, array &$stats): string
    {
        $before = $text;
        // فاصله‌های سه‌تایی یا بیشتر → یکی
        $text = preg_replace('/ {3,}/u', ' ', $text) ?? $text;
        // واژه تکراری پشت‌سرهم («واقعا واقعا»، «خیلی خیلی») → یک بار
        // (هر دو شکل با/بدون تنوین — چون لایه املایی قبل از اینجا تنوین می‌گذارد)
        $text = preg_replace('/(?<![\p{L}\p{N}])((?:خیلی|واقعاً?|کاملاً?|دقیقاً?|حتماً?|احتمالاً?|معمولاً?|بسیار|همیشه)) +\1(?![\p{L}\p{N}])/u', '$1', $text) ?? $text;
        // «اینکه که» و «که اینکه»
        $text = preg_replace('/اینکه +که/u', 'اینکه', $text) ?? $text;
        // «را که را»
        $text = preg_replace('/را +که +را/u', 'را که', $text) ?? $text;
        if ($text !== $before) {
            $stats['redundancy']++;
        }
        return $text;
    ...
    private static function fixSpelling(string $text, array &$stats): string
    {
        ...
        if ($map === null) {
            ...
            $map = [
                ...
                // ارجاع جغرافیایی
                $b . 'امرار' . $e => 'عبور',
                /* 🆕 v1.2: واژگان تخصصی تعمیرات لوازم خانگی */
                $b . 'لباس شویی' . $e => 'لباسشویی',
                $b . 'ظرف شویی' . $e => 'ظرفشویی',
                $b . 'جارو برقی' . $e => 'جاروبرقی',
                $b . 'پکیچ' . $e => 'پکیج',
                $b . 'میکروویو' . $e => 'مایکروویو',
                $b . 'تکنیسین' . $e => 'تکنسین',
                $b . 'تکنشن' . $e => 'تکنسین',
                $b . 'گرانتی' . $e => 'گارانتی',
                $b . 'برسی' . $e => 'بررسی',
                $b . 'کمپرسسور' . $e => 'کمپرسور',
                $b . 'کاندانسور' . $e => 'کندانسور',
                $b . 'سنسسر' . $e => 'سنسور',
                $b . 'ترموستت' . $e => 'ترموستات',
                $b . 'ترمستات' . $e => 'ترموستات',
                $b . 'دیفروست' . $e => 'دیفراست',
                $b . 'درین' . $e => 'تخلیه',
                $b . 'دریناژ' . $e => 'تخلیه',
                $b . 'بخاطر' . $e => 'به دلیل',
                $b . 'ماشین لباس شویی' . $e => 'ماشین لباسشویی',
                $b . 'ماشین ظرف شویی' . $e => 'ماشین ظرفشویی',
                $b . 'یخچال فریژر' . $e => 'یخچال‌فریزر',
                $b . 'فریژر' . $e => 'فریزر',
                $b . 'ساید بای ساید' . $e => 'سایدبایساید',
                $b . 'نوفراست' . $e => 'نوفروست',
                $b . 'نو فراست' . $e => 'نوفروست',
            ];
        }
        $before = $text;
        foreach ($map as $pattern => $replacement) {
